...
**Notes**:
- Workflows created **inactive** (must publish separately)
...
> **GOTCHA — `active` and `tags` are read-only:** MCP update tools modify the draft version only.
> The `active` field and `tags` cannot be set via MCP. To activate a workflow or change tags,
> use `PUT /api/v1/workflows/{id}` via REST API directly (HTTP Request node or curl).

> **GOTCHA — Webhook nodes via MCP MUST include `webhookId`:** When creating webhook nodes
> via MCP (`addNode`), always include a `webhookId` field in the node parameters.
> Without it, the webhook endpoint won't register and the workflow will silently fail to trigger.
...
5. ACTIVATE (when ready)
   ⚠️ **IMPORTANT LIMITATION**: Workflow publishing/activation is NOT supported via MCP.
   ...
   **Manual publishing required**: Navigate to workflow in n8n UI and toggle activation.
...
**Publishing Note**: After creating and validating workflows via MCP, inform users they must:
1. Open the workflow in n8n UI (provide workflow ID)
2. Review the workflow configuration
3. Publish the workflow, then toggle activation in the n8n UI
