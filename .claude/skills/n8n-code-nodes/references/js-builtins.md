...
> isolated task runner sandbox. If your n8n instance uses task runners (default in v2.0+),
> this function will throw a runtime error. **Use HTTP Request nodes instead.**
...
> isolated task runner sandbox. `crypto.randomUUID()` is also unavailable.
> For hashing/HMAC, use n8n's native webhook Header Auth or a pre-webhook HTTP Request node.
> For UUIDs, use this Math.random() alternative:
> ```javascript
...
**Workaround**: Use HTTP Request node (recommended for v2.0+ task runners). $helpers.httpRequest() only works in legacy mode without task runners.
